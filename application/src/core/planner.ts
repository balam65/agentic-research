...
import { SituationalAwareness } from '../memory/state_view.js';
import type { ArtifactKind, WorldView } from '../memory/schema.js';
  ...
  async scoreCapabilities(world: WorldView, capabilities: CapabilityModule[]): Promise<CapabilityDecision[]> {
    ...
    const scored = await Promise.all(
      capabilities.map(async (capability) => {
        ...
        const outputsMatchGoal = capability.descriptor.outputs.some((output) => missingGoals.includes(output as ArtifactKind));
        return {
          capability,
